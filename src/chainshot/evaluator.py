...
def interestingness_score(result: SolveResult, metrics: LevelMetrics) -> float:
    """Provisional CHAIN SHOT identity score, v0.2.

    True chain capacity starts only after the first cue-to-object collision:
    collision_count 0/1/2/3 maps to chain capacity 0/0/1/2.
    """
    built_chain = _clamp01(metrics.built_chain_gain / 2.0)
    peak_chain = _clamp01(metrics.max_chain_capacity / 2.0)
    ...
    # CHAIN SHOT identity penalties. A single collision is not a chain.
    if metrics.max_chain_capacity == 0:
        score -= 18.0
    if metrics.built_chain_gain == 0:
        score -= 8.0
    if (
        metrics.setup_shots == 0
        and metrics.built_chain_gain == 0
        and metrics.max_chain_capacity == 0
    ):
        score -= 12.0
